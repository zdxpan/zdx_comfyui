...
class Refinerv1Pro():
    ...
    FUNCTION = "call"
    DESCRIPTION = """
refiner image use v1.5 tile controlnet and ipdapter
"""
    ...
    def initiate(self):
        ...
        self.ipadapternoise = NODE_CLASS_MAPPINGS["IPAdapterNoise"]()
...
#-------load and apply ipadapter0---------
        ...
        if reference_image is not None:
            ...
            ipadapterunifiedloader_27 = self.ipadapterunifiedloader.load_models(
                preset="PLUS (hight strength)",
                model=get_value_at_index(self.automatic_cfg_688, 0),
            )
            ipadapternoise_29 = self.ipadapternoise.make_noise(
                type="shuffle",
                strength=0.5,
                blur=0,
                image_optional=get_value_at_index(reference_image, 0),
            )
            ipadapteradvanced_15 = self.ipadapteradvanced.apply_ipadapter(
                weight=1.0,
                ...
                model=get_value_at_index(ipadapterunifiedloader_27, 0),
                ipadapter=get_value_at_index(ipadapterunifiedloader_27, 1),
                image=get_value_at_index(ipadapternoise_29, 0),
                attn_mask=reference_mask,
            )
            ipadapterunifiedloader_17 = self.ipadapterunifiedloader.load_models(
                preset="LIGHT - SD1.5 only (low strength)",
                model=get_value_at_index(ipadapteradvanced_15, 0),
            )
            ipadapteradvanced_16 = self.ipadapteradvanced.apply_ipadapter(
                weight=1.0,
                weight_type="style transfer precise",
                combine_embeds="concat",
                start_at=0,
                end_at=1,
                embeds_scaling="K+mean(V) w/ C penalty",
                model=get_value_at_index(ipadapterunifiedloader_17, 0),
                ipadapter=get_value_at_index(ipadapterunifiedloader_17, 1),
                image=get_value_at_index(ipadapternoise_29, 0),
                attn_mask=reference_mask,
            )
            final_model = get_value_at_index(ipadapteradvanced_16, 0)
