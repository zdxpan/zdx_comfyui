...
def log(message:str, message_type:str='info'):
    ...
    print(f"😺zdx_nodes: {name} -> {message}")
...
def shift_image(image:Image, distance_x:int, distance_y:int, background_color:str='#000000', cyclic:bool=False) -> Image:
    if cyclic:
        return ImageChops.offset(image, -distance_x, -distance_y)
    else:
        width, height = image.size
        ret_image = Image.new('RGB', (width, height), color=background_color)
        ret_image.paste(image, (-distance_x, -distance_y))
        return ret_image
...
def create_box_gradient(start_color_inhex:str, end_color_inhex:str, width:int, height:int, scale:int=50) -> Image:
    ...
    start_color = np.array(Hex_to_RGB(start_color_inhex))
    end_color = np.array(Hex_to_RGB(end_color_inhex))
    
    step = int(min(width, height) * scale / 100 / 2)
    
    # Create grid of distances to border
    x = np.arange(width)
    y = np.arange(height)
    xv, yv = np.meshgrid(x, y)
    
    # Distance to nearest border
    dist_x = np.minimum(xv, width - 1 - xv)
    dist_y = np.minimum(yv, height - 1 - yv)
    dist = np.minimum(dist_x, dist_y)
    
    # Initialize with end_color (center area)
    arr = np.full((height, width, 3), end_color, dtype=np.uint8)
    
    if step > 0:
        # mask for gradient area (border)
        mask_grad = dist < step
        
        # Calculate gradient factors
        # dist goes from 0 (edge) to step-1 (inner)
        # color = start * (step - dist)/step + end * dist/step
        # factor = dist / step
        factors = dist[mask_grad].astype(np.float32) / step
        factors = factors[:, np.newaxis] # (N, 1)
        
        # Calculate gradient colors
        grad_colors = start_color + (end_color - start_color) * factors
        arr[mask_grad] = grad_colors.astype(np.uint8)
...
def create_gradient(start_color_inhex:str, end_color_inhex:str, width:int, height:int, direction:str='bottom') -> Image:
    ...
    if direction == 'bottom':
        ...
        gradient = np.linspace(end_color, start_color, height)
        arr[:] = gradient[:, np.newaxis, :]
        ...
        gradient = np.linspace(end_color, start_color, width)
        arr[:] = gradient[np.newaxis, :, :]
    else:
...
def histogram_range(image:Image, black_point:int, black_range:int, white_point:int, white_range:int) -> Image:
    ...
    if black_point == 255: black_point = 254
    if white_point == 0: white_point = 1
    if black_point + black_range > 255: black_range = 255 - black_point
    if white_range > white_point: white_range = white_point
    
    if black_point == white_point:
        return Image.new("L", size=image.size, color="white")

    img_arr = np.array(image, dtype=np.float32)
    
    # White part
    white_part = np.zeros_like(img_arr, dtype=np.uint8)
    # > white_point -> 255
    white_part[img_arr > white_point] = 255
    # Range
    if white_range > 0:
        mask = (img_arr > (white_point - white_range)) & (img_arr <= white_point)
        # val = (pixel - (wp - wr)) / wr * 255
        val = (img_arr[mask] - (white_point - white_range)) / white_range * 255
        white_part[mask] = val.astype(np.uint8)
    
    white_part = 255 - white_part # Invert

    # Black part
    black_part = np.zeros_like(img_arr, dtype=np.uint8)
    # < black_point -> 255
    black_part[img_arr < black_point] = 255
    # Range
    if black_range > 0:
        mask = (img_arr < (black_point + black_range)) & (img_arr >= black_point)
        # val = (pixel - bp) / br * 255
        val = (img_arr[mask] - black_point) / black_range * 255
        # putpixel 255 - val
        black_part[mask] = 255 - val.astype(np.uint8)

    black_part = 255 - black_part # Invert
    
    # Combine (Darken)
    ret_arr = np.minimum(white_part, black_part)
...
def histogram_equalization(image:Image, mask:Image=None, gamma_strength=0.5) -> Image:
    ...
    if mask is not None:
        ...
        valid_pixels = img_arr.flatten()

    if valid_pixels.size == 0:
        log(f"histogram_equalization: mask is not available, return orinianl image.")
        return image
        
    min_brightness = np.min(valid_pixels)
    max_brightness = np.max(valid_pixels)
    average_brightness = np.mean(valid_pixels)

    # Remap entire image
    # (pixel - min) / (max - min) * 255
    if max_brightness > min_brightness:
        img_arr = (img_arr.astype(np.float32) - min_brightness) / (max_brightness - min_brightness) * 255
        img_arr = np.clip(img_arr, 0, 255).astype(np.uint8)
    else:
        # If max == min, image is flat, cannot equalize range
        pass 
